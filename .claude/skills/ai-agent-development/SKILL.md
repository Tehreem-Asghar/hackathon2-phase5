...
## Description
A comprehensive skill for developing AI agents that integrate with your application. This skill provides guidance and templates for creating intelligent agents that can interact with your database, handle user requests, and perform complex tasks using tools.

## Features
- Agent configuration and setup
- Tool creation and integration
- External AI service integration (Gemini/OpenAI)
...
### 3. Agent Integration in API Routes
```python
...
async def chat(
    *,
    ...
    # Actually history_msgs from select will NOT include the new user_msg unless we refresh/commit.
    # So history_msgs from select will NOT include the new user_msg.
...
1. **Environment Configuration**: Always use environment variables for API keys and model configurations
2. **Error Handling**: Implement proper error handling with graceful fallbacks
3. **Async Operations**: Use async/await for database operations and API calls
4. **User Isolation**: Ensure agents operate within user-specific contexts
5. **Tool Design**: Create focused, well-documented tools with proper error handling
6. **Rate Limiting**: Handle API rate limits gracefully in production
7. **Security**: Validate inputs and sanitize outputs from AI agents
8. **Logging**: Implement proper logging for debugging and monitoring

## Common Patterns

1. **Agent Factory Pattern**: Create agent instances per user request with user-specific context
2. **History Management**: Implement sliding window for conversation history to manage context length
3. **Tool Validation**: Validate tool parameters before executing database operations
4. **Fallback Strategies**: Have fallback mechanisms when primary AI service is unavailable
5. **Session Management**: Properly manage database sessions in async environments

## Troubleshooting

1. **API Key Issues**: Ensure GEMINI_API_KEY or OPENAI_API_KEY is properly set in environment
2. **Async Context Issues**: Use nest_asyncio when running in Jupyter or other async contexts
3. **Database Connection**: Verify database connection settings and async session management
4. **Tool Execution**: Check that all required parameters are passed to tools
5. **Rate Limits**: Handle 429 errors gracefully with retry mechanisms